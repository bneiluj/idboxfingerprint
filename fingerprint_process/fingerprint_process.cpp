...
JNIEXPORT jfloat Java_io_idbox_fpsample_FingerprintWrapper_nativeCompare(JNIEnv *env, jobject thiz, jbyteArray capture, jbyteArray check, jint width, jint height) {

    jbyte* _capture  = env->GetByteArrayElements(capture, 0);
    int len = env->GetArrayLength (capture);
    unsigned char* buf = new unsigned char[len];
    env->GetByteArrayRegion (capture, 0, len, reinterpret_cast<jbyte*>(buf));
    Mat rawData (cv::Size(width,height), CV_8UC3, buf );
    cv::Mat input =imdecode(rawData,IMREAD_GRAYSCALE);

    jbyte* _check  = env->GetByteArrayElements(check, 0);
    int lenc = env->GetArrayLength (check);
    unsigned char* bufc = new unsigned char[lenc];
    env->GetByteArrayRegion (capture, 0, lenc, reinterpret_cast<jbyte*>(bufc));
    Mat rawDatac (cv::Size(width,height), CV_8UC3, bufc );
    cv::Mat input2 =imdecode(rawDatac,IMREAD_GRAYSCALE);
    ...
    env->ReleaseByteArrayElements(capture, _capture, 0);
    env->ReleaseByteArrayElements(check, _check, 0);
